...
│   └── images/
│       └── projects/
│           ├── src/           ← Place original project images here (optimize-images input)
│           └── ...             ← generated by scripts/sync-content-images.ts, gitignored
├── src/
│   ├── animations/
...
New project images go through the usual pipeline (see [Image Optimisation](#image-optimisation)), except the optimized output is committed into `content/images/projects/` instead of `public/images/projects/` (which is now build-generated — see below).
...
This uses **Sharp** to generate responsive variants at 400 w, 800 w, and 1200 w in both WebP and JPEG formats. Output lands in `public/images/projects/` — from there, copy the new variants into `content/images/projects/` and commit them to the `portfolio-content` repository (see [Customising Content](#customising-content)); `public/images/projects/` itself is gitignored and regenerated from `content/` on every build by `scripts/sync-content-images.ts`.
